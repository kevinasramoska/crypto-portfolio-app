...
export default function PriceTable({
  ...
  const totalValue = holdings.reduce((acc, holding) => acc + holding.currentValueUsd, 0);
        ...
        <tbody className="bg-gray-950/40 text-sm text-gray-100">
          {holdings.map(holding => (
            <tr key={holding.id} className="border-t border-gray-900/40">
              ...
              <td className="px-6 py-4 text-right">{currencyFormatter.format(holding.currentPriceUsd)}</td>
              <td className="px-6 py-4 text-right">{currencyFormatter.format(holding.investedValueUsd)}</td>
              <td className="px-6 py-4 text-right">{currencyFormatter.format(holding.currentValueUsd)}</td>
              <td className="px-6 py-4 text-right">{currencyFormatter.format(holding.profitLossUsd)}</td>
            </tr>
          ))}
        </tbody>
