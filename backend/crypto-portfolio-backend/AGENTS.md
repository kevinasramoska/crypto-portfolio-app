...
- **Testing**: Integration tests with `@SpringBootTest`, MockMvc for endpoints, `@MockBean` for external services like `MarketDataService`
- **Flyway Migrations**: Schema changes in `V1__init_schema.sql`, tables: users, coins, holdings, transactions, user_roles
...
- **Tests**: `./mvnw test` (includes integration tests with test profile)
...
Reference files: `pom.xml` for dependencies, `application.yaml` for config, `V1__init_schema.sql` for DB schema.</content>
<parameter name="filePath">/Users/kevinasramoska/Desktop/crypto-portfolio-app/backend/crypto-portfolio-backend/AGENTS.md
